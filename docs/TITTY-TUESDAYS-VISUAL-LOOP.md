...
> - **Providers.** OpenAI `gpt-image-2` at medium quality and fal `fal-ai/flux/schnell`, declared
>   as `TT_VISUAL_IMAGE` and `TT_VISUAL_IMAGE_FAL`. Gemini stays deferred.
> - **Storage.** Bytes live under `state/ventures/titty-tuesdays/design-proposals/media/`, served
