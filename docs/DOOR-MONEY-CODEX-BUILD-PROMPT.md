...
The program is cut into GitHub issues **`DM-00` – `DM-22c`** on `lukaskourilcz/quorum`
(#193–#240), one issue per deliverable. The issue list refines this document's task
...
> Work the Door Money venture program in `lukaskourilcz/quorum`. First read
> `docs/DOOR-MONEY-CODEX-BUILD-PROMPT.md` and `docs/DOOR-MONEY-VENTURE-DESIGN.md`
> in full. Then work ONLY the GitHub issues whose titles start with `DM-`, **one
> issue at a time, in title order**, starting with `DM-00` (the audit). Never
> touch another program's issues (`KV-`, `BH-`, `TS-`). Commit frequently: **at
> least one commit per issue, never more than one issue per commit**; tick the
> issue's checkbox in the founding decision in the commit that completes it; close
> each issue with a one-line comment naming the commit(s); keep the full gate
> green at every phase boundary. Never commit book text beyond the excerpt caps —
> that is the program's one irreversible mistake. Where an issue and the documents
> disagree: founding decision > design doc > build prompt > issue. Record every
