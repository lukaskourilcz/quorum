...
- [ ] **Tick `HUMAN_APPROVAL DEVSHARK-SOCIAL-003` in `state/INBOX.md` before creating any account** — it approves the Buffer Free account, the Meta app and its four publish scopes, and Actions write on `BOARDLESSAI_GITHUB_TOKEN`, and says where each value lives. [imp:5] [owner:me] [time:10m] [kind:decision]
...
- [ ] **Tick `HUMAN_APPROVAL DEVSHARK-SOCIAL-001` and then `-002` in `state/INBOX.md`** when the connections are live and three drafted packages exist. Both are written under Pending with their scope text; the scope text is the approval, and you tick each in your own commit. [imp:5] [owner:me] [time:10m] [kind:decision]
...
- [ ] **Import devShark's coding challenges for the Wednesday teaser** once devShark ships its difficulty labels (its step D5, `difficultyOf` in `shared/coding-catalog.ts`): run `pnpm marketingshark:import-challenges -- --brand devshark --source ../react-express-app` from the root of a clean quorum clone with devShark cloned beside it (`--source` is read from the directory the command is typed in), and commit `state/marketingshark/challenge-banks/devshark.json` (#576). Until then every Wednesday drafts the quiz and its record says why; today the import refuses because D5 has not shipped. [imp:3] [owner:ai] [time:15m] [kind:content]
