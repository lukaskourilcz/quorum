...
- [x] **Lift the `smol-toml` pin Dependabot cannot** — done 2026-09-26: `"smol-toml@<1.7.1": 1.7.1` in the overrides of `pnpm-workspace.yaml` takes the Vercel CLI's exact 1.5.2 pin to the fixed 1.7.1, and the lockfile now resolves 1.7.1 only. Deploy-time tooling, not runtime. The `pnpm deploy:check` you run before the next deploy covers the CLI with it. [imp:2] [owner:ai] [time:30m] [kind:setup]
