...
| Apify | No MMA ledger exists and no actor may run: both `APIFY-ACCOUNT-001` and `APIFY-MMA-SOURCES-001` are pending. [Evidence: absent `state/ventures/fightaiq/source-quota/apify.json`; `state/INBOX.md:8-31`, `state/INBOX.md:63-82`] | The existing shared module refuses a missing token before a request. Its current GoVIRAL constants describe the whole $5 free credit and a $1.40 reservation; Q4 must add a separate $3 MMA share without weakening either layer. [Evidence: `orchestrator/src/sources/apify.ts:20-39`, `orchestrator/src/sources/apify.ts:98-140`, `state/INBOX.md:67-78`] |

The planned MMA Apify path has $0 cash authority. The owner estimate is $1.50–$3.00 of free platform credit per month: a Cheerio-class run is roughly $0.02–$0.05, while a paid-per-result UFC actor at the stated cadence is roughly $1.80/month. Q4 must replace estimates with pinned per-actor pricing evidence before any actor’s first run. [Evidence: `state/INBOX.md:63-82`; pricing fields and terms gate `orchestrator/src/sources/apify.ts:43-55`]
