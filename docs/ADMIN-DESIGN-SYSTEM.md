...
The server resolves every navigation entry from the existing company views and recorded portfolio ventures before the interactive boundary. Company Overview, Operations, Implementation Plans, Social Profiles, Approvals, Only you can do, Future, Design Lab, and each workspace keep canonical URLs, so reload, copy, browser back, and bookmarks preserve context. Social Profiles uses one protected `/admin/social-profiles` route with stable `section`, `profile` and `campaign` query state; Campaigns extends that route instead of creating a competing Admin or scheduler surface. The Queue (`/admin/queue`, issue #573) is a Company destination between Waiting for you and Settings, badged with the number of waiting posts; its `status`, `venture` and `platform` filters are query state, and `docs/SOCIAL-QUEUE.md` documents it. Design Lab is grouped under Production without changing its stored `carousel-studio` identifier; its `brand` and `article` query state open one section and one article (issue #575). No duplicate route is created for system panels that already live in Company Overview.
