...
- **Capability allowlist** — Door Money has exactly three content/service relationships:
  a held `goviral -> door-money: intelligence-read` packet, a
  `door-money -> design-lab: bounded-render-summary` handoff and a
  `door-money -> social-distribution: approved-publish-package` reference. The Social
  Distribution edge still requires its own connection, provider and routine authority.
  No manuscript, chunk, embedding, private annotation, style exemplar or private-store
  payload can cross any edge. All other venture and owner-workspace content/data edges are
  denied.
- **Shared services** — Design Lab supplies deterministic rendering. Treasury, scheduler,
  progress and owner-attention services remain generic infrastructure, not content
  exchange. Model calls ride the budget ledger under `ledgerNamespace: door-money`; there
  is no cash spend anywhere in the design.
