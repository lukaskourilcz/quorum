...
describe("Carousel Studio gallery and showcase", () => {
  it("exposes ten checked live seed templates across three brands and formats", async () => {
    const snapshot = await readCarouselStudio();
    expect(snapshot.templates).toHaveLength(10);
    expect(snapshot.templates.every((entry) => entry.template.status === "live" && entry.allChecksPass)).toBe(true);
    expect(snapshot.brands.map((brand) => brand.id)).toEqual(["caught-up", "mma-files", "titty-tuesdays"]);
