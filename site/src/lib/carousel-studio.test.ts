...
describe("Carousel Studio gallery and showcase", () => {
  it("exposes twelve checked live seed templates across six brands and formats", async () => {
    ...
    expect(snapshot.brands.map((brand) => brand.id)).toEqual(["caught-up", "mma-files", "titty-tuesdays", "devshark", "geoshark", "door-money"]);
