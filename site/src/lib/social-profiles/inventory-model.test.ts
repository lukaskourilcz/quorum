...
describe("Social content inventory admin parsers", () => {
  it("accepts every canonical profile strategy and the valid inventory fixture", async () => {
    ...
    // Eleven since devShark's three platform profiles gained their constitutions (quorum#569).
    expect(registry.strategies.map(parseSocialProfileStrategy)).toHaveLength(11);
