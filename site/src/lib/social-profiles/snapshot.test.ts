...
describe("Social Profiles server snapshot", () => {
  it("keeps real groups, capability truth and missing metrics honest", async () => {
    ...
    // Eleven: devShark's three platform profiles joined the eight in quorum#569, and parse as
    // readily as the rest (the dropped counts below stay zero).
    expect(snapshot.ventureProfiles).toHaveLength(11);
    ...
    expect(snapshot.providerControl).toMatchObject({ summary: { directCoreAvailable: true, activeBindings: 0, heldBindings: 9, ambiguousReceipts: 0 }, authorityGranted: false, purchaseAuthorized: false, automaticFailover: false });
    expect(snapshot.contentRunway).toMatchObject({ summary: { strategies: 11, healthy: 0, lowOrNoRunway: 0, unavailable: 11, actualCostUsd: 0 }, authorityGranted: false, queueAuthorized: false, publishingAuthorized: false });
  ...
  it("loads the shared fifty-profile matrix only for an explicit non-production request", async () => {
    ...
    expect(preview.ventureProfiles).toHaveLength(11);
  ...
  it("projects a validated no-candidate inventory and receipt without forcing a post", async () => {
    ...
    const snapshot = await readAdminSocialProfiles(root, { environment: { NODE_ENV: "test" } });
    expect(snapshot.contentRunway.summary).toEqual({ strategies: 11, healthy: 0, lowOrNoRunway: 1, unavailable: 10, actualCostUsd: 0 });
