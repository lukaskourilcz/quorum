...
const statusLabels: Record<PublicIdeaStatus, string> = {
  proposed: "Suggested",
  accepted: "Approved",
      ...
      <section className="mx-auto max-w-[var(--container)] px-5 py-18 md:px-10 md:py-24">
        <div aria-label="Filter ideas by status" className="flex flex-wrap gap-2">
          ...
          {ideaStatuses.map((ideaStatus) => (
            <Link className={buttonVariants({ size: "small", variant: selected === ideaStatus ? "accent" : "secondary" })} href={`/ideas?status=${ideaStatus}`} key={ideaStatus} scroll={false}>{statusLabels[ideaStatus]}</Link>
          ))}
        </div>
        ...
        {visible.length ? (
          <div className="mt-10 rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-5 md:p-8">
            <Table>
              <thead><tr><TableHead>Idea</TableHead><TableHead>Current status</TableHead><TableHead>First suggested in</TableHead><TableHead>Status history</TableHead><TableHead>Related ideas</TableHead></tr></thead>
              <tbody>
                {visible.map((idea) => {
                  ...
                  return (
                    <tr id={idea.id} key={idea.id}>
                      ...
                      <TableCell><ol className="grid min-w-60 gap-3">{idea.statusHistory.map((event, index) => { const href = publicMeetingHref(event.meetingRef, standups); return <li className="border-l-2 border-[var(--border)] pl-3" key={`${event.at}-${index}`}><div className="flex flex-wrap items-center gap-2"><Badge tone={tone(event.status)}>{statusLabels[event.status]}</Badge><time className="font-mono text-[0.625rem] text-[var(--fog)]" dateTime={event.at}>{formatDate(event.at.slice(0, 10))}</time></div><p className="mt-2 text-xs leading-5 text-[var(--fog)]">{event.reason}</p>{href ? <Link className="mt-1 inline-block text-xs text-[var(--accent)] underline underline-offset-4" href={href}>Decision meeting</Link> : null}</li>; })}</ol></TableCell>
                      <TableCell><div className="grid gap-2">{idea.revival ? <Link className="text-sm text-[var(--accent)] underline underline-offset-4" href={`/ideas#${idea.revival.from}`}>Reopens {idea.revival.from}</Link> : null}{idea.supersededBy ? <Link className="text-sm text-[var(--accent)] underline underline-offset-4" href={`/ideas#${idea.supersededBy}`}>Replaced by {idea.supersededBy}</Link> : null}{idea.similarTo.map((link) => <Link className="text-sm text-[var(--fog)] underline underline-offset-4" href={`/ideas#${link.id}`} key={`${link.id}-${link.verdict}`}>{link.verdict.replace("_", " ")} · {Math.round(link.score * 100)}% match</Link>)}{!idea.revival && !idea.supersededBy && idea.similarTo.length === 0 ? <span className="text-sm text-[var(--fog)]">No related earlier idea</span> : null}</div></TableCell>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div className="mt-10 rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-10"><Badge>No saved ideas</Badge><h2 className="mt-5 text-3xl font-semibold tracking-[-0.04em]">There is nothing to show here yet.</h2><p className="mt-4 max-w-2xl text-base leading-7 text-[var(--fog)]">No product idea has been saved{selected ? ` with the status “${statusLabels[selected]}”` : ""}. We do not add fake examples just to fill the page.</p></div>
        )}
      </section>
    </PageShell>
  );
