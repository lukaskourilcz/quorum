...
import {
  applyTehdejsiFeatureAction,
  parseTehdejsiFeatureAction,
  TehdejsiFeaturePersistenceError
} from "@/lib/tehdejsi-features-store";
...
export async function POST(request: Request): Promise<Response> {
  ...
  const action = parseTehdejsiFeatureAction(payload);
  if (!action) return json({ error: "Feature action is invalid or incomplete." }, 422);

  try {
    const result = await applyTehdejsiFeatureAction(action);
    return json(result, result.idempotent ? 200 : 201);
  } catch (error) {
    if (error instanceof TehdejsiFeaturePersistenceError) {
