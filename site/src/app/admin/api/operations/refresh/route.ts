...
import { OperationsRefreshError, requestOperationsRefresh } from "@/lib/admin-operations-refresh";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const receipt = await requestOperationsRefresh({ requestedBy: process.env.ADMIN_USER?.trim() || "owner" });
    return json({ ok: true, receipt }, 202);
  } catch (error) {
    if (error instanceof OperationsRefreshError) {
      const status = error.code === "COOLDOWN" ? 429 : error.code === "CONFLICT" ? 409 : 503;
      return json({ error: error.message, code: error.code }, status);
    }
    return json({ error: "The Operations refresh request was not recorded." }, 500);
  }
}
