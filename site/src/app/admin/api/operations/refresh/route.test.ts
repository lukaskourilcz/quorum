...
function request(options: { auth?: boolean; origin?: string; body?: string } = {}): Request {
  ...
  return new Request(`${ORIGIN}/admin/api/operations/refresh`, { method: "POST", headers, body: options.body });
}

beforeEach(async () => {
  root = await mkdtemp(path.join(os.tmpdir(), "operations-refresh-route-"));
  vi.useFakeTimers();
...
describe("the Operations refresh route", () => {
  it("authenticates and records a bounded request for the next checkpoint", async () => {
    ...
    expect(JSON.parse(await readFile(path.join(root, "state/operations/refresh-request.json"), "utf8")))
      .toMatchObject({ schemaVersion: "operations-refresh-request/1", requestedBy: "owner" });
  });
