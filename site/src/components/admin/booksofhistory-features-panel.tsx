...
function FeatureReview({ feature, snapshot }: { feature: AdminBhFeature; snapshot: AdminBooksofhistorySnapshot }) {
  ...
  return (
    <article className="min-w-0 max-w-full overflow-hidden rounded-[12px] border border-[var(--admin-border-strong)] bg-[var(--admin-surface-inset)] p-4" data-bh-feature={feature.recommendationId}>
      ...
      {feature.designLabStatus === "ready" || feature.designLabStatus === "rendered" ? <Link className="mt-5 inline-flex min-h-11 items-center font-semibold text-[var(--admin-section-accent)] underline underline-offset-4" href="/admin?venture=carousel-studio&tab=studio">Open both locale records in Design Lab →</Link> : null}
