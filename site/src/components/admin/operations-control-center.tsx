...
function IncidentsRecovery({ snapshot }: { snapshot: AdminOperationsSnapshot }) {
  ...
  return (
    <div className="grid min-w-0 gap-5">
      ...
      {incidents.state === "present" ? <AdminCard><AdminCardHeader><AdminSectionHeading description="Only references and bounded policy metadata cross the server boundary." title="Recovery evidence" /></AdminCardHeader><AdminCardContent className="grid gap-2 text-[length:var(--admin-type-control)]"><p className="m-0">Recent attempts: {incidents.recentAttemptRefs.length} · policies: {incidents.policyVersions.join(", ") || "none recorded"}</p><p className="m-0">Affected nodes: {incidents.affectedNodeIds.join(", ") || "none"} · unaffected nodes: {incidents.unaffectedNodeIds.join(", ") || "none recorded"}</p><p className="m-0">Exact owner actions: {incidents.exactOwnerActions.length} · paused scopes: {incidents.pausedScopes.length}</p></AdminCardContent></AdminCard> : null}
...
function PlansProgress({ snapshot }: { snapshot: AdminOperationsSnapshot }) {
  ...
  return (
    <div className="grid min-w-0 gap-5">
      ...
      <AdminCard><AdminCardHeader><AdminSectionHeading description={`Generated ${formatDate(progress.generatedAt)} · source ${progress.sourceFreshness}`} title="Canonical implementation snapshot" /></AdminCardHeader><AdminCardContent><p className="m-0">A short version of one progress snapshot. Work items, probes, blockers, pull requests and the release gates are under Implementation plans.</p></AdminCardContent><div className="border-t border-[var(--admin-border)] px-4 py-3"><Link className="admin-focus-ring rounded-sm font-semibold text-[var(--admin-link)] underline-offset-4 hover:underline" href="/admin/implementation-plans">Open Implementation Plans →</Link></div></AdminCard>
    </div>
  );
}
