  ...
  return (
    <header className={cn("flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between", className)}>
      <div className="min-w-0">
        {eyebrow ? <p className="m-0 text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--admin-foreground-muted)]">{eyebrow}</p> : null}
        <h1 className="m-0 text-[22px] font-semibold tracking-[-0.025em] text-[var(--admin-foreground)]">{title}</h1>
        {description ? <p className="m-0 mt-1 max-w-3xl text-[13px] text-[var(--admin-foreground-muted)]">{description}</p> : null}
      </div>
...
export function AdminSectionHeading({
  ...
  return (
    <div className={cn("flex min-w-0 items-start justify-between gap-3", className)}>
      <div className="min-w-0">
        <h2 className="m-0 text-[14px] font-semibold tracking-[-0.01em] text-[var(--admin-foreground)]">{title}</h2>
        {description ? <p className="m-0 mt-0.5 text-[12px] text-[var(--admin-foreground-muted)]">{description}</p> : null}
      </div>
...
export function AdminCardHeader({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return <div className={cn("border-b border-[var(--admin-border)] px-4 py-3", className)} {...props} />;
}
...
export function AdminCardFooter({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return <div className={cn("border-t border-[var(--admin-border)] px-4 py-3", className)} {...props} />;
}
...
export function AdminMetric({
  ...
  return (
    <div className={cn("min-w-0 bg-[var(--admin-surface)] px-4 py-3.5", className)} style={style}>
      <p className="m-0 text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--admin-foreground-muted)]">{label}</p>
      <p className="admin-tabular m-0 mt-1.5 text-[24px] font-semibold tracking-[-0.035em] text-[var(--admin-foreground)]">{value}</p>
      ...
      {note ? <p className="m-0 mt-1.5 text-[11px] text-[var(--admin-foreground-muted)]">{note}</p> : null}
    </div>
  );
}

export const adminButtonVariants = cva(
  "admin-focus-ring inline-flex min-h-[var(--admin-touch-target)] items-center justify-center gap-1.5 rounded-[var(--admin-radius)] border px-3 text-[12px] font-semibold transition-colors duration-[var(--admin-motion-fast)] disabled:pointer-events-none disabled:opacity-50 md:min-h-[var(--admin-control-height)]",
  {
    variants: {
      variant: {
...
const fieldControl =
  "admin-focus-ring min-h-[var(--admin-touch-target)] w-full rounded-[var(--admin-radius)] border border-[var(--admin-border-strong)] bg-[var(--admin-surface-inset)] px-3 text-[13px] text-[var(--admin-foreground)] placeholder:text-[var(--admin-foreground-subtle)] disabled:cursor-not-allowed disabled:opacity-55 md:min-h-[var(--admin-control-height)]";

export function AdminLabel({ className, ...props }: LabelHTMLAttributes<HTMLLabelElement>) {
  return <label className={cn("mb-1 block text-[12px] font-medium text-[var(--admin-foreground)]", className)} {...props} />;
}
...
export function AdminEntityBadge({ className, children, ...props }: HTMLAttributes<HTMLSpanElement>) {
  return (
    <span
      className={cn(
        "inline-flex min-h-6 items-center rounded-[var(--admin-radius-sm)] border border-[var(--admin-border)] bg-[var(--admin-surface-secondary)] px-2 py-0.5 text-[11px] font-medium text-[var(--admin-foreground-muted)]",
        className,
      )}
      {...props}
    >
...
export function AdminEmptyState({
  ...
  return (
    <div className={cn("flex min-h-40 flex-col items-center justify-center rounded-[var(--admin-radius)] border border-dashed border-[var(--admin-border-strong)] bg-[var(--admin-surface-secondary)] p-6 text-center", className)}>
      ...
      <p className="m-0 text-[13px] font-semibold text-[var(--admin-foreground)]">{title}</p>
      {description ? <p className="m-0 mt-1 max-w-md text-[12px] text-[var(--admin-foreground-muted)]">{description}</p> : null}
...
export function AdminTable({ className, ...props }: TableHTMLAttributes<HTMLTableElement>) {
  return <table className={cn("w-full border-collapse text-left text-[12px]", className)} {...props} />;
}

export function AdminTableHead({ className, ...props }: ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      className={cn("h-[var(--admin-row-dense)] border-b border-[var(--admin-border)] bg-[var(--admin-surface-secondary)] px-3 text-[10px] font-semibold uppercase tracking-[0.06em] text-[var(--admin-foreground-muted)]", className)}
      {...props}
    />
  );
}
