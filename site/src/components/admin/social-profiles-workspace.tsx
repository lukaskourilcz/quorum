...
function Today({ snapshot }: { snapshot: AdminSocialProfilesSnapshot }) {
  const candidateByRef = new Map<string, { candidateType: string; formatId: string }>(snapshot.contentRunway.profiles.flatMap(({ inventory }) => (inventory?.candidates ?? []).map((candidate): [string, { candidateType: string; formatId: string }] => [`state/social/inventory-candidates/${candidate.id}.json`, { candidateType: candidate.candidateType, formatId: candidate.formatId }])));
  const ownerAction = (outcome: string, scopeState: string, providerState: string) => outcome === "NO_POST"
    ? "No action. Do not force filler."
    : outcome === "queued"
      ? "Pause or cancel before provider pickup; retain the immutable receipt."
      : providerState === "ambiguous"
        ? "Reconcile #409/#417 evidence before any retry."
        : scopeState !== "matched"
          ? "Review the exact item; countersign or revoke only the exact bounded scope, then request dry re-evaluation."
          : "Repair the named held gate or veto the item, then request dry re-evaluation.";
  return (
    <div className="grid gap-4" data-social-profiles-section="today">
      <AdminCard><AdminCardHeader><AdminSectionHeading title={`Today · ${snapshot.today.targetDate}`} description="One deterministic Prague-day decision per real profile connection. Inventory and #410 campaign decisions are consumed by reference; this view never generates content or publishes." /></AdminCardHeader><AdminCardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <AdminMetric label="Queued" value={snapshot.today.summary.queued} note="Exact #409 handoffs" />
        <AdminMetric label="Review" value={snapshot.today.summary.review} note="Owner evidence required" />
        <AdminMetric label="Held / paused" value={snapshot.today.summary.held + snapshot.today.summary.paused} note="No silent failover" />
        <AdminMetric label="NO_POST" value={snapshot.today.summary.noPost} note="Explicit healthy outcome" />
        <AdminMetric label="Owner attention" value={snapshot.today.summary.attention} note="Bounded evidence refs" />
        <AdminMetric label="Recorded cost" value={`$${snapshot.today.summary.actualCostUsd.toFixed(2)}`} note="No inferred spend" />
      </AdminCardContent></AdminCard>
      <AdminCard><AdminCardHeader><AdminSectionHeading title={`Daily operations · ${snapshot.today.operations.length}`} description="Selected type/ref, useful window, gate receipts, routine scope, queue/provider state and replay identity remain inspectable without secret values." /></AdminCardHeader><AdminCardContent>
        {snapshot.today.operations.length ? <AdminTableRegion label="Social daily operations"><AdminTable><thead><tr><AdminTableHead>Profile / connection</AdminTableHead><AdminTableHead>Selected item / window</AdminTableHead><AdminTableHead>Outcome / reasons</AdminTableHead><AdminTableHead>Gate evidence</AdminTableHead><AdminTableHead>Scope / queue / provider</AdminTableHead><AdminTableHead>Owner-safe next action</AdminTableHead><AdminTableHead>Receipt / replay</AdminTableHead></tr></thead><tbody>{snapshot.today.operations.map((operation) => {
          const candidate = operation.selectedCandidateRef ? candidateByRef.get(operation.selectedCandidateRef) : null; const blocked = operation.gates.filter(({ status }) => status !== "pass");
          return <tr key={operation.id}><AdminTableCell><span className="block font-semibold">{operation.profileId}</span><span className="block font-mono text-[length:var(--admin-type-micro)] text-[var(--admin-foreground-muted)]">{operation.connectionId}</span></AdminTableCell><AdminTableCell>{candidate ? `${candidate.candidateType} · ${candidate.formatId}` : operation.selectedCandidateRef ? "Referenced candidate unavailable" : "No selection"}<span className="block break-all font-mono text-[length:var(--admin-type-micro)] text-[var(--admin-foreground-muted)]">{operation.selectedCandidateRef ?? "NO_POST / preselection hold"}</span><span className="mt-1 block text-[length:var(--admin-type-label)]">{operation.selectionWindow.notBefore} → {operation.selectionWindow.notAfter}</span></AdminTableCell><AdminTableCell><AdminStatusBadge tone={statusTone(operation.outcome)}>{operation.outcome}</AdminStatusBadge><span className="mt-1 block text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">{operation.reasons.join(" · ")}</span></AdminTableCell><AdminTableCell>{blocked.length ? blocked.map(({ gate, status, reason }) => <span className="mb-1 block" key={gate}><AdminStatusBadge tone={statusTone(status)}>{gate}: {status}</AdminStatusBadge><span className="mt-1 block text-[length:var(--admin-type-micro)] text-[var(--admin-foreground-muted)]">{reason}</span></span>) : <AdminStatusBadge tone="success">all recorded gates pass</AdminStatusBadge>}</AdminTableCell><AdminTableCell><span className="block">Scope <AdminStatusBadge tone={statusTone(operation.routineScopeState)}>{operation.routineScopeState}</AdminStatusBadge></span><span className="mt-1 block">Provider <AdminStatusBadge tone={statusTone(operation.providerConnectionState)}>{operation.providerConnectionState}</AdminStatusBadge></span><span className="mt-1 block break-all font-mono text-[length:var(--admin-type-micro)] text-[var(--admin-foreground-muted)]">{operation.queue?.itemRef ?? "No queue handoff"}</span></AdminTableCell><AdminTableCell>{ownerAction(operation.outcome, operation.routineScopeState, operation.providerConnectionState)}</AdminTableCell><AdminTableCell><span className="block font-mono text-[length:var(--admin-type-micro)]">{operation.id}</span><span className="mt-1 block">{operation.replayed ? "Replay recognized" : "Canonical first decision"}</span></AdminTableCell></tr>;
        })}</tbody></AdminTable></AdminTableRegion> : <AdminEmptyState title="No daily operation receipts" description="No valid decision exists for this Prague date. Absence is shown as unavailable, not as a zero-post success or an invented held item." />}
      </AdminCardContent></AdminCard>
      <AdminCard><AdminCardHeader><AdminSectionHeading title={`Routine scopes · ${snapshot.today.routineScopes.length}`} description="Draft-only is the repository default. Only one exact active, countersigned profile/connection scope may allow queue handoff; no wildcard or implied authority is shown." /></AdminCardHeader><AdminCardContent>
        {snapshot.today.routineScopes.length ? <AdminTableRegion label="Social routine scopes"><AdminTable><thead><tr><AdminTableHead>Profile / connection</AdminTableHead><AdminTableHead>Status / dates</AdminTableHead><AdminTableHead>Content boundary</AdminTableHead><AdminTableHead>Evidence</AdminTableHead><AdminTableHead>Owner action</AdminTableHead></tr></thead><tbody>{snapshot.today.routineScopes.map((scope) => <tr key={scope.id}><AdminTableCell><span className="block font-semibold">{scope.profileId}</span><span className="block font-mono text-[length:var(--admin-type-micro)]">{scope.connectionId}</span></AdminTableCell><AdminTableCell><AdminStatusBadge tone={statusTone(scope.status)}>{scope.status}</AdminStatusBadge><span className="mt-1 block">{scope.effectiveOn} → {scope.expiresOn}</span></AdminTableCell><AdminTableCell>{scope.platform} · {scope.contentClasses.join(", ")}<span className="block text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">{scope.formats.join(", ")} · {scope.sourceKinds.join(", ")}</span></AdminTableCell><AdminTableCell>{scope.countersignatureRef ?? "Not countersigned"}<span className="block text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">{scope.latestAction} · {scope.latestAt}</span></AdminTableCell><AdminTableCell>{scope.status === "active" ? "Revoke the exact scope with owner evidence; pending queue items remain separately cancellable." : "No live queue authority. Review or leave draft-only."}</AdminTableCell></tr>)}</tbody></AdminTable></AdminTableRegion> : <AdminEmptyState title="No countersigned routine scopes" description="The canonical registry is empty and draft-only. A prepared item may reach review, but cannot enter the queue without a later exact owner countersignature." />}
      </AdminCardContent></AdminCard>
      <AdminCallout tone="information">NO_POST is a healthy explicit outcome. Do not force filler. Owner-safe actions are bounded to exact-item approval or veto, dry re-evaluation, exact-scope revocation, and pause/cancel. None bypasses #409 publishing, #417 provider, capability, budget, kill-switch or ambiguous-delivery gates.</AdminCallout>
    </div>
  );
}
...
function displayMetric(value: number | null, suffix = ""): string {
  return value === null ? "Unavailable" : `${value.toLocaleString("en-US", { maximumFractionDigits: 4 })}${suffix}`;
}

function Results({ snapshot }: { snapshot: AdminSocialProfilesSnapshot }) {
  const results = snapshot.socialResults;
  const ventures = results.profiles.filter(({ targetRole }) => targetRole !== "amplifier");
  const amplifiers = results.profiles.filter(({ targetRole }) => targetRole === "amplifier");
  return (
    <div className="grid gap-4" data-social-profiles-section="results">
      <AdminCard><AdminCardHeader><AdminSectionHeading title="Results" description="Append-only official own-account insights, first-party destination events and owner-entered aggregates. Missing metrics are unavailable, never zero; audience identities and private messages are excluded." /></AdminCardHeader><AdminCardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <AdminMetric label="Observations" value={results.summary.observations} note="24h · 72h · 7d · 28d snapshots" />
        <AdminMetric label="Measured posts" value={results.summary.measuredPosts} note={`${results.summary.unavailablePosts} unavailable`} />
        <AdminMetric label="Attributed events" value={results.summary.attributedEvents} note={`${results.summary.unattributedEvents} remain unattributed/invalid`} />
        <AdminMetric label="28-day baseline" value={results.baselines.at(-1)?.status ?? "Unavailable"} note={results.baselines.at(-1) ? `${results.baselines.at(-1)!.elapsedDays} elapsed days` : "No baseline record"} />
        <AdminMetric label="Live experiments" value={results.summary.activeExperiments} note="Maximum two; hard gates frozen" />
        <AdminMetric label="Recorded cost" value={results.summary.actualCostUsd === null ? "Unavailable" : `$${results.summary.actualCostUsd.toFixed(2)}`} note="Provider/model/tool evidence only" />
      </AdminCardContent></AdminCard>
      <AdminCard><AdminCardHeader><AdminSectionHeading title={`Venture Profile results · ${ventures.length}`} description="Primary and exact-capability umbrella results stay separate from Amplification Profile baselines." /></AdminCardHeader><AdminCardContent>
        {ventures.length ? <AdminTableRegion label="Venture Profile results"><AdminTable><thead><tr><AdminTableHead>Profile / role</AdminTableHead><AdminTableHead>Reliability / sample</AdminTableHead><AdminTableHead>Qualified reach</AdminTableHead><AdminTableHead>Referrals / actions / conversions</AdminTableHead><AdminTableHead>Provider / cost</AdminTableHead></tr></thead><tbody>{ventures.map((profile) => <tr key={profile.profileId}><AdminTableCell><span className="block font-semibold">{profile.profileId}</span><span className="block text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">{profile.targetRole} · #424 {profile.capabilityVersions.join(", ") || "not required"}</span></AdminTableCell><AdminTableCell>{displayMetric(profile.verifiedPublishRate, " rate")}<span className="block text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">{profile.latestPostObservations} post(s) · {profile.unavailablePosts} unavailable</span></AdminTableCell><AdminTableCell>Reach {displayMetric(profile.totals.reach)}<span className="block text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">Views {displayMetric(profile.totals.views)} · likes evidence {displayMetric(profile.totals.likes)}</span></AdminTableCell><AdminTableCell>{displayMetric(profile.totals.referralVisits)} / {displayMetric(profile.totals.qualifiedActions)} / {displayMetric(profile.totals.conversions)}</AdminTableCell><AdminTableCell><AdminStatusBadge tone={statusTone(profile.providerState)}>{profile.providerState}</AdminStatusBadge><span className="mt-1 block">{profile.actualCostUsd === null ? "Cost unavailable" : `$${profile.actualCostUsd.toFixed(2)}`}</span></AdminTableCell></tr>)}</tbody></AdminTable></AdminTableRegion> : <AdminEmptyState title="No Venture Profile results" description="No validated primary or umbrella observation exists. A published-post count or reach value is not inferred." />}
      </AdminCardContent></AdminCard>
      <AdminCard><AdminCardHeader><AdminSectionHeading title={`Amplification Profile results · ${amplifiers.length}`} description="Independent original baseline, venture-support sample, ratio and policy incidents are required before a continue/narrow/pause/retire proposal." /></AdminCardHeader><AdminCardContent>
        {amplifiers.length ? <AdminTableRegion label="Amplification Profile results"><AdminTable><thead><tr><AdminTableHead>Profile</AdminTableHead><AdminTableHead>Original / support</AdminTableHead><AdminTableHead>Ratio / policy</AdminTableHead><AdminTableHead>Referral outcomes</AdminTableHead><AdminTableHead>Decision evidence</AdminTableHead></tr></thead><tbody>{amplifiers.map((profile) => <tr key={profile.profileId}><AdminTableCell><span className="font-semibold">{profile.profileId}</span></AdminTableCell><AdminTableCell>{profile.originalSample} / {profile.supportSample}</AdminTableCell><AdminTableCell>{displayMetric(profile.originalRatio)} / {displayMetric(profile.supportRatio)}<span className="block text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">{profile.policyIncidents} incident(s)</span></AdminTableCell><AdminTableCell>{displayMetric(profile.totals.referralVisits)} visits · {displayMetric(profile.totals.qualifiedActions)} qualified · {displayMetric(profile.totals.conversions)} converted</AdminTableCell><AdminTableCell><AdminStatusBadge tone={profile.amplifierDecisionEvidence === "available" ? "success" : "warning"}>{profile.amplifierDecisionEvidence}</AdminStatusBadge><span className="mt-1 block text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">sample {profile.latestPostObservations} · cost {profile.actualCostUsd === null ? "unavailable" : `$${profile.actualCostUsd.toFixed(2)}`}</span></AdminTableCell></tr>)}</tbody></AdminTable></AdminTableRegion> : <AdminEmptyState title="No Amplification Profile results" description="No accepted Amplification Profile exists, so no ordinary baseline or support outcome is invented." />}
      </AdminCardContent></AdminCard>
      <AdminCard><AdminCardHeader><AdminSectionHeading title={`Campaign result sets · ${results.campaigns.length}`} description="Results remain grouped by source venture, target role, platform, format and locale; primary-only is an explicit baseline." /></AdminCardHeader><AdminCardContent>
        {results.campaigns.length ? <AdminTableRegion label="Social campaign results"><AdminTable><thead><tr><AdminTableHead>Campaign / release</AdminTableHead><AdminTableHead>Roles / sample</AdminTableHead><AdminTableHead>Platform / format / locale</AdminTableHead><AdminTableHead>Referral outcomes</AdminTableHead><AdminTableHead>Time / cost / state</AdminTableHead></tr></thead><tbody>{results.campaigns.map((campaign) => <tr key={campaign.campaignRef}><AdminTableCell><span className="block font-semibold">{campaign.sourceVentureId}</span><span className="block font-mono text-[length:var(--admin-type-label)] text-[var(--admin-foreground-muted)]">{campaign.releaseId}</span></AdminTableCell><AdminTableCell>{campaign.primaryOnly ? "primary-only" : campaign.targetRoles.join(" + ")} · {campaign.sample}</AdminTableCell><AdminTableCell>{campaign.platforms.join("/")} · {campaign.formats.join("/")} · {campaign.locales.join("/")}</AdminTableCell><AdminTableCell>{campaign.referralVisits} / {campaign.qualifiedActions} / {campaign.conversions}</AdminTableCell><AdminTableCell>{displayMetric(campaign.timeToDistributeSeconds, "s")} · {campaign.actualCostUsd === null ? "cost unavailable" : `$${campaign.actualCostUsd.toFixed(2)}`}<span className="block"><AdminStatusBadge tone={statusTone(campaign.state)}>{campaign.state}</AdminStatusBadge></span></AdminTableCell></tr>)}</tbody></AdminTable></AdminTableRegion> : <AdminEmptyState title="No Campaign result sets" description="No validated observation is linked to a current #410 campaign. Campaign preparation remains available even when providers or analytics fail." />}
      </AdminCardContent></AdminCard>
      <AdminCard><AdminCardHeader><AdminSectionHeading title="Baseline, experiments & held proposals" description="A complete 28-day organic baseline precedes any experiment. At most two may be live; boost evidence can only create a non-executing owner proposal." /></AdminCardHeader><AdminCardContent className="grid gap-4 lg:grid-cols-3">
        <AdminCallout tone={results.baselines.at(-1)?.status === "complete" ? "success" : "neutral"}><p className="m-0 font-semibold">Organic baseline</p><p className="m-0 mt-1">{results.baselines.at(-1) ? `${results.baselines.at(-1)!.startsOn} → ${results.baselines.at(-1)!.endsOn} · ${results.baselines.at(-1)!.status}` : "Unavailable; no winner is declared."}</p></AdminCallout>
        <AdminCallout tone={results.experiments.some(({ status }) => status === "active" || status === "review") ? "information" : "neutral"}><p className="m-0 font-semibold">Experiments · {results.experiments.length}</p><p className="m-0 mt-1">{results.experiments.length ? results.experiments.map(({ changedVariable, verdict, evidenceCount, minimumSample }) => `${changedVariable}: ${verdict} (${evidenceCount}/${minimumSample})`).join(" · ") : "No experiment is active or inferred."}</p></AdminCallout>
        <AdminCallout tone={results.boostProposals.length ? "warning" : "neutral"}><p className="m-0 font-semibold">Held boost proposals · {results.boostProposals.length}</p><p className="m-0 mt-1">{results.boostProposals.length ? "Owner decision required; no ad API, purchase or spend authority exists." : "No organic result has created a proposal."}</p></AdminCallout>
      </AdminCardContent></AdminCard>
    </div>
  );
}
