...
export function MmaFilesAdminPanel({ snapshot, tab }: { snapshot: AdminMmaFilesSnapshot; tab: Tab }) {
  if (tab === "articles") return <div className="mt-8 grid gap-7">
    {snapshot.articles.length ? snapshot.articles.map((article) => <Card key={article.id}><CardContent>
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs uppercase text-[var(--fog)]">{article.date} · {article.slot.toUpperCase()}</p><p className="mt-1 text-sm text-[var(--fog)]">Both languages come from one tamper-checked saved article.</p></div><Badge tone={statusTone(article.status)}>{statusLabel(article.status)}</Badge></div>
  ...
  if (tab === "calendar") return <div className="mt-8 grid gap-5">
    {snapshot.calendar.length ? snapshot.calendar.map((day) => <Card key={day.date}><CardContent><h3 className="text-2xl font-semibold">{day.date}</h3><div className="mt-5 grid gap-4 md:grid-cols-2">{day.slots.map((slot) => { const state = slot.articleStatus ?? slot.status; return <div className="rounded-[var(--radius-button)] border border-[var(--border)] bg-[var(--surface)] p-5" key={slot.slot}><div className="flex items-center justify-between gap-3"><strong className="uppercase">{slot.slot}</strong><Badge tone={statusTone(state)}>{statusLabel(state === "killed" ? "killed" : slot.articleStatus ?? "not run")}</Badge></div><p className="mt-3 font-semibold">{slot.format.replaceAll("-", " ")}</p><p className="mt-2 text-sm leading-6 text-[var(--fog)]">{slot.killedReason ?? slot.rationale}</p></div>; })}</div></CardContent></Card>) : <Callout>No daily article plan is stored yet. A missed slot will appear here as missed, not as an empty article.</Callout>}
  </div>;
