...
export function Dialog({
  ...
  return createPortal(
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-[rgba(6,6,8,.78)] backdrop-blur-[6px] motion-safe:[animation:bai-dialog-in_160ms_ease-out]"
      ...
      <div
        ...
        aria-modal="true"
        className="relative flex max-h-full w-full max-w-[min(920px,100%)] flex-col overflow-hidden rounded-[14px] border border-[#3f3f46] bg-[rgba(11,11,13,.98)] shadow-[0_40px_120px_rgba(0,0,0,.7)] outline-none motion-safe:[animation:bai-dialog-in_200ms_cubic-bezier(.22,.61,.36,1)]"
        data-dialog-surface
        ref={surface}
        role="dialog"
        tabIndex={-1}
      >
        <div className="flex items-start gap-4 border-b border-[#26262b] px-5 py-4 md:px-7">
          <div className="min-w-0">
            {eyebrow ? (
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#94949c]">{eyebrow}</p>
            ) : null}
            <h2 className="mt-1 text-[19px] font-semibold tracking-[-0.02em] text-[#f4f4f5]" id={titleId}>
              {title}
            </h2>
          </div>
          <button
            aria-label="Close"
            className="ml-auto shrink-0 rounded-[9px] border border-[#3f3f46] bg-[#101013] px-2.5 py-1 font-mono text-[13px] leading-none text-[#d4d4d8] transition-colors hover:border-[#a1a1aa] hover:text-[#f4f4f5]"
            data-dialog-close
            onClick={close}
            type="button"
          >
        ...
        */}
        <div className="min-w-0 flex-1 overflow-y-auto px-5 py-5 md:px-7" data-dialog-body data-wheel-exempt>
          {children}
        </div>
        {footer ? (
          <div className="border-t border-[#26262b] px-5 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-[#94949c] md:px-7">
            {footer}
          </div>
        ) : null}
      </div>
