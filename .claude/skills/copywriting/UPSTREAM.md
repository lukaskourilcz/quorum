...
Vendored verbatim and mirrored byte-for-byte into `.agents/skills/copywriting/`.
...
Only `copywriting` is vendored from that pair. Upstream's `copy-editing` skill is
deliberately absent: it would sit alongside `stop-slop`, which is this
repository's prose contract, and two editors disagreeing about the same sentence
is worse than one.
