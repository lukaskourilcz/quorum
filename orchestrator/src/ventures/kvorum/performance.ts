...
async function performanceEvidence(
  ...
  const results = new Map<string, {
    record: z.infer<typeof OwnerResultEntrySchema>;
    stateRef: string;
  }>();
  for (const name of names.filter((entry) => /^\d{4}-\d{2}-\d{2}-[a-f0-9]{12}\.json$/u.test(entry)).sort()) {
    const record = OwnerResultEntrySchema.parse(JSON.parse(
  ...
  for (const resultId of [...wanted].sort()) {
    ...
    const recommendation = VentureRecommendationSchema.parse(await readJson<unknown>(
      root,
