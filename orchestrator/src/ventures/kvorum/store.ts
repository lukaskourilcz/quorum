...
import {
  ...
  VentureRecommendationStatusSchema,
...
async function loadRecommendationFiles(root: string): Promise<Array<{
  ...
  for (const filename of filenames.filter((entry) => /^\d{4}-\d{2}-\d{2}-.+\.json$/u.test(entry)).sort()) {
    ...
    records.push({
      filename,
      relativePath,
      recommendation: VentureRecommendationSchema.parse(JSON.parse(
...
function buildIndex(
  date: string,
  now: Date,
  files: readonly { relativePath: string; recommendation: VentureRecommendation }[]
...
export async function writeKvorumRecommendationDay(input: {
  ...
  recommendations: VentureRecommendation[];
  ...
  return withFileLock(input.root, `${RECOMMENDATION_DIRECTORY}/.lock`, async () => {
    ...
    const recommendations: VentureRecommendation[] = [];
