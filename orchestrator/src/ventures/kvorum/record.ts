...
import { KVORUM_REGISTER_GATE_IDS } from "./content-gates.js";
...
export function buildKvorumDeskMeetingRecord(input: {
  ...
  const gateResults = input.outcome.gateEvaluations.flatMap((evaluation) => evaluation.results);
  const hacekResults = gateResults.filter((result) => KVORUM_REGISTER_GATE_IDS.has(result.gate));
  const hacekParticipated = hacekResults.length > 0;
  const hacekVeto = hacekResults.some((result) => result.verdict === "fail");
  ...
  return MeetingRecordSchema.parse({
    ...
    participantReasons: [
      ...
      {
        agent: "HACEK",
        reason: hacekParticipated
          ? "the deterministic Czech alarm, voter-respect and stop-slop register gates evaluated the returned candidates"
          : "no schema-valid candidate reached the deterministic Czech register gates",
        participated: hacekParticipated
      },
      {
        agent: "AUDIT",
        reason: input.outcome.gateEvaluations.length > 0
          ? "the deterministic evidence and banned-content gates evaluated every returned candidate"
    ...
    voteMatrix: input.outcome.gateEvaluations.length > 0
      ? [
        ...(hacekParticipated ? [{
          voter: "HACEK" as const,
          firstChoice: hacekVeto ? "veto:register" : "pass",
          veto: hacekVeto
        }] : []),
        {
          voter: "AUDIT",
    ...
    roomTranscript: {
      ...
      turns: [
        ...(hacekParticipated
          ? [{
              agent: "HACEK" as const,
              mode: (hacekVeto ? "veto" : "statement") as "veto" | "statement",
              sentAt: at,
              text: hacekVeto
                ? "At least one returned candidate failed the deterministic Czech register floor and cannot proceed."
                : "Every schema-valid candidate passed the deterministic Czech register floor."
            }]
          : []),
