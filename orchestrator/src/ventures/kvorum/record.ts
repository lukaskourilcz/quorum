...
import type { TribunPackage } from "../../contracts/kvorum-desk.js";
import {
  buildCalendarFeed,
...
function outcomeSummary(outcome: KvorumDeskRecordOutcome): string {
  if (outcome.status === "packages") {
    return `${outcome.packages.length} owner-review draft ${outcome.packages.length === 1 ? "package" : "packages"}: ${outcome.packages.map((candidate) => candidate.headline).join("; ")}.`;
  }
...
export function buildKvorumDeskMeetingRecord(input: {
  ...
  return MeetingRecordSchema.parse({
    ...
    participantReasons: [
      ...
      { agent: "HACEK", reason: "the deterministic register gate lands in KV-13 and was not claimed here", participated: false },
      { agent: "AUDIT", reason: "the deterministic truth gates land in KV-13 and were not claimed here", participated: false }
    ],
    ledger: {
    ...
    roomTranscript: {
      ...
      turns: [{ agent: "TRIBUN", mode: "close", sentAt: at, text: summary, evidenceRefs: refs }]
    },
    generatedAt: at
  });
}
