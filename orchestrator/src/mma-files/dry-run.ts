...
const dryGateway: MmaFilesEditorialGateway = {
  async writeEnglish({ slot }) {
    return slot === "am" ? {
      ...
      title: "Ewa Example enters the fixture fighter file",
      dek: "This dry profile checks the same evidence rules as a live newsroom slot.",
      bodyMDX: "## Fixture profile\n\n[Ewa Example](/fighters/ksw/ewa-example) carries an 8-1 fixture record. [^source-1]\n\n[Anna Example](/fighters/ksw/anna-example) carries a 7-2 fixture record. [^source-1]"
    };
  },
  async localizeCzech({ slot }) {
    return slot === "am" ? {
      ...
      title: "Ewa Example vstupuje do zkušebního profilu bojovnice",
      dek: "Suchý profil hlídá stejná pravidla pro zdroje jako živá redakce.",
      bodyMDX: "## Zkušební profil\n\n[Ewa Example](/fighters/ksw/ewa-example) má zkušební bilanci 8-1. [^source-1]\n\n[Anna Example](/fighters/ksw/anna-example) má zkušební bilanci 7-2. [^source-1]"
    };
  }
};
  ...
  return produceMmaFilesArticle({
    ...
    evidence: {
      ...
      fighterRefs: morning
        ...
        : ["ksw:ewa-example", "ksw:anna-example"],
      ...(morning ? { eventRef: "ufc:event:fixture-prague" } : {}),
      heroSpec: {
        template: morning ? "tale-of-the-tape" : "fighter-file",
        bindings: {
          headline: morning ? "Alex Example vs Sam Example" : "Ewa Example",
          fixture: true
        }
      },
