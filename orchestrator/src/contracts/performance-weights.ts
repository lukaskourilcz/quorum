...
const BhWeightSchema = z.number().min(0).max(10);
const BhProposalIdSchema = z.string().regex(/^weights-[a-f0-9]{20}$/);
const BhResultIdSchema = z.string().regex(/^result-[a-f0-9]{20}$/);

export const BooksofHistoryPerformanceWeightsSchema = z.strictObject({
  ...
  lanes: z.record(BhSlugSchema, z.record(BhSlugSchema, z.record(BhSlugSchema, BhWeightSchema))),
...
export const BooksofHistoryPerformanceWeightProposalSchema = z.strictObject({
  ...
  adjustments: z.array(z.strictObject({
    lane: BhSlugSchema,
    dimension: BhSlugSchema,
    key: BhSlugSchema,
    from: BhWeightSchema,
    to: BhWeightSchema,
    resultIds: z.array(BhResultIdSchema).min(1).max(20),
