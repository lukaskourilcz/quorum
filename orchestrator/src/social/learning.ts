...
function latest28dByPost(values: readonly unknown[], profileId: string, evaluatedAt: Date, maximumLookbackDays: number): SocialMetricObservation[] {
  const byPost = new Map<string, SocialMetricObservation>();
  const earliest = evaluatedAt.getTime() - maximumLookbackDays * 86_400_000;
  for (const value of values) {
    const parsed = SocialMetricObservationSchema.safeParse(value); if (!parsed.success || parsed.data.profileId !== profileId || parsed.data.maturityWindow !== "28d" || Date.parse(parsed.data.publishedAt) < earliest || Date.parse(parsed.data.observedAt) > evaluatedAt.getTime()) continue;
...
export function evaluateSocialProfileLearning(input: { profileId: string; targetRole: "primary" | "umbrella" | "amplifier"; strategy: unknown; observations: readonly unknown[]; operations: readonly unknown[]; experiments: readonly unknown[]; policy: unknown; evaluatedAt: Date }): { evaluation: SocialLearningEvaluation; adjustment: SocialStrategyAdjustment | null } {
  ...
  const observations = latest28dByPost(input.observations, input.profileId, input.evaluatedAt, policy.maximumLookbackDays); const operations = input.operations.flatMap((value) => { const parsed = SocialProfileOperationSchema.safeParse(value); return parsed.success && parsed.data.profileId === input.profileId && Date.parse(parsed.data.createdAt) >= input.evaluatedAt.getTime() - policy.maximumLookbackDays * 86_400_000 && Date.parse(parsed.data.createdAt) <= input.evaluatedAt.getTime() ? [parsed.data] : []; }).sort((left, right) => left.targetDate.localeCompare(right.targetDate));
