...
describe("Kvórum bounded performance weights", () => {
  ...
  test("the one writer resolves stored owner receipts and is byte-idempotent", async () => {
    ...
    const [recommendationRaw, resultRaw, monitorRaw, weightsRaw] = await Promise.all([
      readFile(path.join(repoRoot, "contracts/fixtures/kvorum-venture-recommendation.valid.json"), "utf8"),
      readFile(path.join(repoRoot, "contracts/fixtures/kvorum-owner-result-entry.valid.json"), "utf8"),
    ...
    KvorumRecommendationSchema.parse(recommendation);
    ...
    for (const [index, resultId] of resultIds.entries()) {
      ...
      KvorumOwnerResultEntrySchema.parse(result);
