...
vi.mock("../src/images/fighter-photo.js", async () => {
  ...
  return {
    ...
    fighterIdentityPhoto: async ({ wikidataId }: { wikidataId: string }) => {
      ...
      // Q104839627 is the OKTAGON fighter's real item and carries no P18, so this is what the
      // live lookup returns for him: there is no photograph.
      return null;
    }
  };
});
...
describe("choosing how to find a picture for an article", () => {
  ...
  it("gives a fighter we hold no card for the FRAME cover, not a search on their name", async () => {
    ...
    const root = await plantGustavoLopez();
    expect(await articleImageCandidates(root, ["oktagon:alexander-gladkov"])).toEqual([]);
    expect(identityCalls).toEqual([]);
    expect(searchCalls).toEqual([]);
  });

  it("gives a misspelt fighter ref the FRAME cover too", async () => {
    ...
    const root = await plantGustavoLopez();
    expect(await articleImageCandidates(root, ["ufc:gustavo-lopez"])).toEqual([]);
    expect(identityCalls).toEqual([]);
    expect(searchCalls).toEqual([]);
  });
