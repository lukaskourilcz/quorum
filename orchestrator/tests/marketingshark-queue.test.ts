...
describe("marketingShark's queue drafts", () => {
  ...
  it("never resolves to an eligible target from the committed registry", async () => {
    ...
    for (const item of items) {
      ...
      // #569 registers devShark's three profiles and connections held.
      expect(resolution.decision, item.channel).toBe("held");
    }
  });

  it("is held on registered devShark connections, and eligible only once the owner activates one", async () => {
    const registered = await loadSocialPublisherRegistry(configRoot);
  ...
  it("is denied without an exact, current reference to marketingShark's edge", async () => {
    const active = activated(await loadSocialPublisherRegistry(configRoot));
...
describe("marketingShark in the legacy queue mapping", () => {
  it("maps a v1 marketingShark item onto devShark's per-platform profiles", async () => {
    const mapped = await loadSocialPublisherRegistry(configRoot);
  ...
  it("is registered in the committed registry, on devShark's own Instagram and Threads connections", async () => {
    // #568 needed the mapping and #569 the profiles it names; a v1 marketingShark item left in the
    // queue must migrate rather than throw inside the migration audit.
    const registry = await loadSocialPublisherRegistry(configRoot);
    expect(registry.legacyQueueMappings.find((mapping) => mapping.venture === "marketingshark")).toEqual({
    ...
    for (const platform of ["instagram", "threads"] as const) {
      const connection = registry.connections.find((candidate) => candidate.id === DEVSHARK_SOCIAL_TARGETS[platform].connectionBindingRef);
      const profile = registry.profiles.find((candidate) => candidate.id === connection?.profileId);
      expect(connection?.platform, platform).toBe(platform);
      expect(profile).toMatchObject({ ventureRef: "marketingshark", role: "venture-primary" });
    }
  });
});
