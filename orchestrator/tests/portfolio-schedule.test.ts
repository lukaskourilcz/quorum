...
describe("portfolio schedule and budget gate", () => {
  it("ships collision-free 05, 06, 07, 11, 14, 17, 21 and 22 Prague slots", async () => {
    const registry = await loadVentureRegistry();
    expect(resolveMeetingClock(registry).map((slot) => slot.hour)).toEqual([5, 6, 7, 11, 14, 17, 21, 22]);
